...
"""
        ...
        if not variables:
            return "print('Error: No number provided for square root')"
        return f"import math\nprint(math.sqrt({variables[0]}))"

    elif operation == "factorial":
        if not variables:
            return "print('Error: No number provided for factorial')"
        return f"import math\nprint(math.factorial({variables[0]}))"
    ...
    operation_map = {
        ...
        "multiply": f"print({var1} * {var2})",
        "divide": f"print({var1} / {var2}) if {var2} != 0 else print('Error: Division by zero')",
        "modulus": f"print({var1} % {var2})",
        "power": f"print({var1} ** {var2})",
        "gcd": f"import math\nprint(math.gcd({var1}, {var2}))",
        "lcm": f"import math\ndef lcm(x, y): return abs(x * y) // math.gcd(x, y)\nprint(lcm({var1}, {var2}))"
    }

    return operation_map.get(operation, "print('Error: Unsupported operation')")
